...
```
![image](https://user-images.githubusercontent.com/105834310/169694358-47f44c80-08e0-4e92-b62f-3d7726253dcc.png)
```python
...
result1 = pd.DataFrame({"max_depth":sorted(md_settings*6), "min_sample_split":mss_settings*6, "training accuracy": training_accuracy, "validation accuracy": validation_accuracy})

result1

max = result1['validation accuracy'].max()

print(max)
```
0.9414893617021277
```python
result1.loc[result1['validation accuracy'].idxmax()]
```
max_depth              10.000000\
min_sample_split        2.000000\
training accuracy       0.997337\
validation accuracy     0.941489\
Name: 24, dtype: float64
```python
# model test
clf1 = DecisionTreeClassifier(max_depth = 10, min_samples_split = 2, random_state=0)
...
tree.plot_tree(clf1)
```
![image](https://user-images.githubusercontent.com/105834310/169695360-7c20a278-14fc-428f-9a5c-cef65bc547d2.png)
```python
f_i=pd.Series(clf1.feature_importances_, index=X_train.columns)
f_i.sort_values(ascending=False)
import matplotlib.pyplot as plt
f_i.sort_values().plot(kind='barh',figsize=(10,6))
plt.show()
```
![image](https://user-images.githubusercontent.com/105834310/169695378-53821bb7-36bb-4ac1-b455-83550812504a.png)
#### Random Forest
```python
...
n_settings = [1, 2, 5, 10, 20, 30, 50, 100]
for n in n_settings:
    ...
    training_accuracy2.append(accuracy_score(y_train, y_train_hat))
    validation_accuracy2.append(accuracy_score(y_val, y_val_hat))
    
pd.DataFrame({"n_estimators":n_settings, "training accuracy2": training_accuracy2, "validation accuracy2": validation_accuracy2})

test_accuracy = []

n_settings = [30, 50, 100]
for n in n_settings:
...
pd.DataFrame({"n_estimators":n_settings, "test accuracy": test_accuracy})

rf = RandomForestClassifier(n_estimators=30, random_state=0)

print("test set accuraccy : {:.6f}".format(rf.score(X_test, y_test)))
```
test set accuraccy : 0.949074
```python
matrix = confusion_matrix(y_test, y_test_hat)
sns.heatmap(matrix, annot=True, fmt="d")
plt.title('Confusion Matrix')
...
plt.ylabel('Actual Class')
```
![image](https://user-images.githubusercontent.com/105834310/169695484-efdeb2d7-1ef9-40e1-a50f-b99d719c18e6.png)
```python
...
plt.show()
```
![image](https://user-images.githubusercontent.com/105834310/169695505-5cc8e125-d0ce-42cf-8526-e35c04024bdd.png)
...
plt.ylabel('Actual Class')
```
![image](https://user-images.githubusercontent.com/105834310/169695645-45b4fa10-f0f6-48ab-ab9d-44f17aca9beb.png)
#### K Nearest Neighbor
```python
...
plt.ylabel('True')
```
![image](https://user-images.githubusercontent.com/105834310/169695851-3ed917ec-4eae-412c-b80a-274dffe881dd.png)
### 기대효과
